...
import 'package:design_api_database/src/widgets/container_slidable/fractionnally_aligned_sized_box.dart';
import 'package:design_api_database/src/widgets/container_slidable/slidable_dismissal.dart';
...
class SlidableData extends InheritedWidget {
  ...
  /// otherwise null.
  double get actionPaneWidthFactor => directionIsXAxis ? totalActionsExtent : null;

  /// If the [direction] is vertical, returns the [totalActionsExtent]
  /// otherwise null.
  double get actionPaneHeightFactor => directionIsXAxis ? null : totalActionsExtent;
...
class Slidable extends StatefulWidget {
  ...
  Slidable.builder({
    ...
    double fastThreshold,
        ...
        assert(enabled != null),
        assert(dismissal == null || key != null, 'a key must be provided if slideToDismissDelegate is set'),
  ...
  static SlidableState of(BuildContext context) {
    final _SlidableScope scope = context.dependOnInheritedWidgetOfExactType<_SlidableScope>();
    return scope?.state;
  }
...
class SlidableState extends State<Slidable> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin<Slidable> {
  ...
  double get _dismissThreshold {
    if (widget.dismissal == null)
      ...
      return widget.dismissal.dismissThresholds[actionType] ?? _kDismissThreshold;
  }

  bool get _dismissible => widget.dismissal != null && _dismissThreshold < 1.0;

  @override
  bool get wantKeepAlive => !widget.closeOnScroll && (_overallMoveController?.isAnimating == true || _resizeController?.isAnimating == true);

  /// The current actions that have to be shown.
  SlideActionDelegate get _actionDelegate => actionType == SlideActionType.primary ? widget.actionDelegate : widget.secondaryActionDelegate;
  ...
  void _handleDragStart(DragStartDetails details) {
    _dragUnderway = true;
    widget.controller?.activeState = this;
    _dragExtent = _actionsMoveAnimation.value * _actionsDragAxisExtent * _dragExtent.sign;
    ...
    setState(() {
      ...
      if (_actionCount > 0) {
        if (_dismissible && !widget.dismissal.dragDismissible) {
          ...
          _overallMoveController.value = (_dragExtent.abs() / _overallDragAxisExtent).clamp(0.0, _totalActionsExtent);
        } else {
  ...
  void _handleDragEnd(DragEndDetails details) {
    ...
    } else if (_actionsMoveAnimation.value >= widget.showAllActionsThreshold || (shouldOpen && fast)) {
      open();
    } else {
      close();
    }
  ...
  void _startResizeAnimation() {
    ...
    if (dismissal.resizeDuration == null) {
      ...
      _resizeController = AnimationController(duration: dismissal.resizeDuration, vsync: this)
